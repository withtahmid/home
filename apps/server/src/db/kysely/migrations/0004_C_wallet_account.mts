...
export const up = async (db: Kysely<any>): Promise<void> => {
    ...
    await db.schema
        .createIndex("idx_wallet_account_id")
        .on("wallet_account")
        .column("id")
        .execute();

    await db.schema
        .createIndex("idx_wallet_account_wallet_id")
        .on("wallet_account")
        .column("wallet_id")
        .execute();
};
export const down = async (db: Kysely<any>): Promise<void> => {
    await db.schema.dropTable("wallet_account").execute();
    await db.schema.dropIndex("idx_wallet_account_id").execute();
    await db.schema.dropIndex("idx_wallet_account_wallet_id").execute();
};
