...
export const up = async (db: Kysely<any>): Promise<void> => {
    ...
    await db.schema
        .createIndex("idx_wallet_savings_id")
        .on("wallet_savings")
        .column("id")
        .execute();
};
export const down = async (db: Kysely<any>): Promise<void> => {
    await db.schema.dropIndex("idx_wallet_savings_id").execute();

    await db.schema.dropTable("wallet_savings").execute();
};
